...
import {
  ...
  Feather, Hash, ArrowRight, Flame, Sparkles } from
"lucide-react";
...
{ id: "views", label: "Mest lästa", icon: EyeIcon }] as
const;
...
}: {onSelectCategory: (id: string) => void;searchQuery: string;setSearchQuery: (q: string) => void;allThreads: Thread[];}) => {
...
}: {thread: Thread;index: number;likedThreads: Set<string>;savedThreads: Set<string>;toggleLike: (id: string) => void;toggleSave: (id: string) => void;onClick: () => void;}) => {
...
}: {reply: ReplyData;depth: number;replyLikes: Set<string>;toggleReplyLike: (id: string) => void;onQuoteReply: (reply: ReplyData) => void;}) => {
...
}: {thread: Thread;onBack: () => void;likedThreads: Set<string>;toggleLike: (id: string) => void;onAddReply: (threadId: string, content: string, quoted?: {author: string;content: string;}) => void;}) => {
...
}: {selectedCategory: string | null;onSelectCategory: (id: string) => void;onBackToOverview: () => void;view: string;onClose?: () => void;allThreads: Thread[];onNewThread?: () => void;}) => {
...
}: {collapsed: boolean;onToggle: () => void;selectedCategory: string | null;onSelectCategory: (id: string) => void;onBackToOverview: () => void;view: string;allThreads: Thread[];onNewThread: () => void;}) =>
...
const CreateThreadView = ({
...
}: {onSubmit: (thread: {title: string;content: string;category: string;tags: string[];}) => void;onCancel: () => void;defaultCategory: string | null;}) => {
  ...
  const handleSubmit = () => {
    if (!canSubmit) {
      if (!category) toast.error("Välj en kategori");else
      if (title.trim().length < 5) toast.error("Rubriken måste vara minst 5 tecken");else
      if (content.trim().length < 10) toast.error("Brödtexten måste vara minst 10 tecken");
      return;
    }
